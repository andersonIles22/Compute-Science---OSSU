...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    """
    max_dict=freq_dict1 if len(freq_dict1)>=len(freq_dict2) else freq_dict2
    diff_result=0
    all_result=0

    for i in max_dict:
        if i in freq_dict2:
            diff_result+=(abs(freq_dict1[i]-freq_dict2[i]))
            all_result+=(freq_dict1[i]+freq_dict2[i])
        else:
            diff_result+=freq_dict1[i]
            all_result+=freq_dict1[i]
    for i in freq_dict2:
        if i not in freq_dict1:
            diff_result+=freq_dict2[i]
            all_result+=freq_dict2[i]

    return 1-(diff_result/all_result)               
...
def get_tf(file_path):
    ...
    * Think about how we can use get_frequencies from earlier
    """
    all_content=load_file(file_path)
    list_content=text_to_list(all_content)
    result=get_frequencies(list_content)
    return result
